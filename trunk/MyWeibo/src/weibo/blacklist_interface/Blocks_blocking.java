 ...
 * 
 */
public class Blocks_blocking {

	public Blocks_blocking() {
	...
	private void dealing(List<NameValuePair> nvps) {
		String url = "http://api.t.sina.com.cn/blocks/blocking.json";
		// 新建一个POST
		...
		if (200 == rp.getStatusLine().getStatusCode()) {
			try {
				...
				try {
					...
					Log.v("nums_of_blockings", data.length() + "");
					if (data.isNull(0))
						Log.v("isNull", "the blacklist is empty");
					else {
						for (int i = 0; i < data.length(); i++) {
							JSONObject user = data.getJSONObject(i);
							Log.v("user_id", user.getString("id"));
							JSONObject status = user.getJSONObject("status");
							Log.v("status_id", status.getString("id"));
							Log.v("status_text", status.getString("text"));
							if (!status.isNull("retweeted_status")) {
								JSONObject retweeted_status = status
										.getJSONObject("retweeted_status");
								Log.v("retweeted_status_text",
										retweeted_status.getString("text"));
								// 这里还有一个user, holy shit!
							}
						}
					}
