 ...
 * 
 */
public class Blocks_blocking_ids {

	public Blocks_blocking_ids() {
	...
	private void dealing(List<NameValuePair> nvps) {
		String url = "http://api.t.sina.com.cn/blocks/blocking/ids.json";
		// 新建一个POST
		...
		if (200 == rp.getStatusLine().getStatusCode()) {
			try {
				...
				try {
					...
					Log.v("num_of_ids", data.length() + "");
					if (data.length() == 0) {
						Log.v("no_id", "no id in blacklist");
					} else {
						for (int i = 0; i < data.length(); i++) {
							Log.v("id " + i, data.getString(i));
						}
					}
