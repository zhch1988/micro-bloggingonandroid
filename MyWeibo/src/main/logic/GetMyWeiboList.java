...
import weibo.status.User_Timeline;
...
public class GetMyWeiboList extends AsyncTask<Void, Void, Integer> {
	...
	public GetMyWeiboList(Activity context) {
	...
	protected Integer doInBackground(Void... params) {
		...
		List<weibo.constant.Status> status = ut.getUser_Timeline();
		if (status != null) {
	...
	protected void onProgressUpdate(Void... values) {
		...
		if (!this.isCancelled()) {
			if (adapter == null) {
				adapter = new WeiboListAdapter(this.publish_status,
						this.context, this.bitmaps);
				ListView listview = (ListView) context
						.findViewById(R.id.apMsglist);
