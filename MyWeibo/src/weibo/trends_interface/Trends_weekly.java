...
public class Trends_weekly {
	...
	public Trends_weekly() {
		String url = "http://api.t.sina.com.cn/trends/weekly.json";
		// 新建一个POST
		...
		// nvps.add(new BasicNameValuePair("date", date.toString()));
		...
		if (200 == rp.getStatusLine().getStatusCode()) {
			try {
				...
				try {
					...
					JSONObject weekly = data.getJSONObject("trends");
					this.time = weekly.names().getString(0);
					Log.v("time", time);
					JSONArray trends = weekly.getJSONArray(time);
