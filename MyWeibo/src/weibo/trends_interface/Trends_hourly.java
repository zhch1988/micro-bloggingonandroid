...
public class Trends_hourly {
	...
	public Trends_hourly() {
		String url = "http://api.t.sina.com.cn/trends/hourly.json";
		// 新建一个POST
		...
		if (200 == rp.getStatusLine().getStatusCode()) {
			try {
				...
				try {
					...
					JSONObject hourly = data.getJSONObject("trends");
					this.time = hourly.names().getString(0);
					Log.v("time", time);
					JSONArray trends = hourly.getJSONArray(time);
