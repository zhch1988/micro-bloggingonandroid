...
public class Followers_ids {
	 ...
	 *            待查询的用户id
	 */
	public Followers_ids(String user_id) {
		// 添加参数
		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("source", Constant.consumerKey));
	 ...
	 *            ：返回的数量
	 */
	public Followers_ids(String user_id, String count) {
		// 添加参数
		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("source", Constant.consumerKey));
		nvps.add(new BasicNameValuePair("user_id", user_id));
	...
	private void dealing(List<NameValuePair> nvps) {
		String url = "http://api.t.sina.com.cn/followers/ids.json";
		// 新建一个POST
