...
public class Favorites_destroy {
	public Favorites_destroy(String id) {
		// json请求格式
		// api地址
		// 要删除的微博消息ID
		String url = "http://api.t.sina.com.cn/favorites/destroy/" + id
				+ ".json";
		// 新建一个POST
		...
		if (200 == rp.getStatusLine().getStatusCode()) {
			try {
				...
				try {
					...
					String ImgPath = data.getString("text");
					// 输出到日志
					Log.v("text", ImgPath);
					// 对json嵌套解析
					String status = data.getString("user");
